        ...
        return new Promise((resolve, reject) => {
            if (cognitoUser) {
                cognitoUser.getSession(function (err: any, session: any) {
                    if (err) {
                        reject("CognitoUtil: Can't set the credentials:" + err);
                    }else{
                        if(session.isValid()) {
                            resolve(session.getAccessToken().getJwtToken());
                        }else{
                            reject("CognitoUtil: Error en getAccessToken");
                        }
                    }
                });
            }else{
                ...
                cognitoUser.getSession(function (err: any, session: any) {
                    if (err) {
                        ...
                        if (session.isValid()) {
                            ...
                            reject("Error en refresh token: sesión invalida");
                        }
                    }
                });
            }else{
                reject("Error en refresh token");
            }
        });    
    }
}
